...
### Implementation Notes
**Overwrite Warning** This is an "Overwrite" operation. To change just the Username, the API must still re-send the existing Password.
...
**:search (String)**
- Description: The user's input (once cleaned). This string can be a first name, last name, or both separated by a space (eg. "John Doe").
...
### Implementation Notes
**Overwrite Warning** This is an "Overwrite" operation. To change just the Email (or whatever attribute), the API must still re-send the existing FirstName, LastName, and Phone (or whichever ones are remaining unchanged).
