...
### Implementation Notes
**Overwrite Warning:** 
- This is an "Overwrite" operation. To change just the Username, the API must still re-send the existing Password.
...
### Implementation Notes
**Overwrite Warning:** 
- This is an "Overwrite" operation. To change just the Email (or whatever attribute(s)), the API must still re-send the existing FirstName, LastName, and Phone (or whichever ones are remaining unchanged).
